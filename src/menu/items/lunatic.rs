...
pub struct LunaticMenuItem;

impl BasicMenuItemMethods for LunaticMenuItem {
    extern "C" fn get_name(_this: &mut BasicMenuItem, _method_info: unity::prelude::OptionalMethod) -> &'static Il2CppString {
        Mess::get("MID_SYS_Difficulty_Lunatic")
    }
    ...
    extern "C" fn a_call(this: &'static mut BasicMenuItem, _method_info: OptionalMethod) -> BasicMenuResult {
        if !this.is_attribute_disable() {
            ...
            let text = format!(
                ...
                current, 
                Mess::get("MID_Hub_Bed_Difficulty_Change_After").to_string(), 
                Mess::get("MID_SYS_Difficulty_Lunatic").to_string()
            );
            YesNoDialog::bind::<LunaticConfirmDialog>(
                this.menu, 
                text, 
...
pub struct LunaticConfirmDialog;

impl TwoChoiceDialogMethods for LunaticConfirmDialog {
    extern "C" fn on_first_choice(_this: &mut BasicDialogItemYes, _method_info: OptionalMethod) -> BasicMenuResult {
        ...
        update_difficulty(GameUserData::get_difficulty(false), 2);
